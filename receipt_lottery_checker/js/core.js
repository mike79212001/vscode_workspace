...
var numTotal = 0;
// debug
// $.each(all_three_number, function(index, value) {
// 	console.log(value);
// });
...
function isNumberWin(number) {
	var isFound = false;
	$.each(allThreeNumber, function(index, value) {
		if (value === number) {
			isFound = true;
		}
	});
...
function updateWinRate(numWin, numTotal) {
	var statusText = "中獎數 / 總次數 : {0}/{1}";
	$('#statistic').html(statusText.format(numWin, numTotal));
};

function showWinMsg(inputValue) {
	var output = "{0} 中獎!! <br/>";
	$('.displayfield').prepend(output.format(inputValue));
	beep();
	numWin++;
}

function showLoseMsg(inputValue) {
	var output = output = "{0} 沒中!! <br/>";
	$('.displayfield').prepend(output.format(inputValue));
}
...
$( document ).ready(function() {
	...
	$('#input-number > input').on('input', function() {
		if (this.value.length >= 3)  {
			...
			if (isNumberWin(inputValue)) {
				showWinMsg(inputValue);
			} else {
				showLoseMsg(inputValue);
			}
			numTotal++;
			updateWinRate(numWin, numTotal);
			this.value = "";
		}
	});
});
